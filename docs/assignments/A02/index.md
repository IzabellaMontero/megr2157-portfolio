...
The truss members are made from ASTM A500 structural steel, which is considered a ductile material. Ductile materials are capable of undergoing noticeable deformation before completely fracturing. According to the American Institute of Steel Construction (AISC), ASTM A500 Grade C rectangular structural steel has a minimum yield strength of approximately 50 ksi and a minimum tensile strength of approximately 62 ksi. Since the yield strength is lower than the ultimate tensile strength, a tension member would generally begin to yield before reaching tensile fracture under gradually increasing loading.

For the members subjected to tension, the expected failure mode is yielding. As the tensile force in a member increases, the normal stress in the member also increases. If this stress reaches the yield strength of the material, the steel will begin to permanently deform. In this truss design, a safety factor of 3.5 was used when determining the required cross-sectional area. The selected member cross-sectional area of 300 square millimeters is also greater than the calculated minimum required area of 270.6 square millimeters. Therefore, yielding should not occur under the specified design loading. A possible design modification to further reduce the likelihood of yielding would be to increase the cross-sectional area of the members, which would reduce the stress produced by the same axial force.

For members subjected to compression, buckling is an important potential failure mode. A long or slender compression member can suddenly bend sideways and become unstable rather than simply yielding from compressive stress. The likelihood of buckling depends on characteristics such as the length of the member, its cross-sectional geometry, material stiffness, and end conditions. A possible modification to reduce the likelihood of buckling would be to increase the size or stiffness of the compression member or reduce its unsupported length by adding additional bracing.

It is important to note that the original truss-design problem states that compression members can be assumed not to fail by buckling. Therefore, buckling was not included when the original member dimensions were calculated. However, in an actual structural application, buckling would still be an important failure mode to investigate for compression members because structural instability could potentially occur before the material reaches its yield strength.

Part 2 – Pin Connections

The expected failure mode for the connecting pins is shear failure. The project specifies that the pins are subjected to single shear, meaning that the force transferred between the connected truss members acts across one shear plane of the pin. If the shear stress becomes greater than the shear strength of the pin material, the pin could permanently deform or eventually shear across this plane. Engineering guidance for pin-loaded connections identifies pin shear as an important failure mode that must be considered when designing this type of connection.
